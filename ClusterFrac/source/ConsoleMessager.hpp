...
//Get singleton instance of console.
#define CF_CONSOLE cf::ConsoleMessager::getInstance()

//Output message string s to console, with log level L.
#define CF_SAY(s, L) CF_CONSOLE->say(s, L)

//Raise exception with message string s.
#define CF_THROW(s) CF_CONSOLE->except(static_cast<std::string>(s));
...
namespace cf
{
	...
	*/
	...
	public:
		...
		//Master abort flag, used to signal the main thread that it should exit immediately
		//due to an exception thrown by a thread. Typically only set by a call to CF_THROW.
		DLL std::atomic<bool> exceptionThrown;

		//Last error message generated by a call to CF_THROW.
		DLL std::string exceptionMessage;
