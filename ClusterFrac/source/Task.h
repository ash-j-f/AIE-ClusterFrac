...
namespace cf
{
	...
	*/
	...
	public:
		...
		*/
		DLL inline virtual std::string getType() const { return "Task"; }
		...
		*/
		DLL inline unsigned __int64 getInitialTaskID() const { if (initialTaskID == 0) { CF_THROW("This task has no ID."); } return initialTaskID; };
		...
		*/
		DLL std::vector<Task *> split(int count) const;
		...
		* @returns void.
		*/
		DLL void serialize(WorkPacket &p) const;
		...
		*/
		DLL inline unsigned int getMaxTaskTimeMilliseconds() const { return maxTaskTimeMilliseconds; };
		...
		*/
		DLL inline void setMaxTaskTimeMilliseconds(unsigned int n) { if (n == 0) { CF_THROW("Invalid timeout setting for task."); } maxTaskTimeMilliseconds = n; };
